...
def save_quiz(sno, title, file_name, quiz_json, question_count, difficulty):
    ...
    try:
        ...
        cursor.execute('''
        INSERT INTO quizzes (sno, title, file_name, quiz_json, question_count, difficulty)
        VALUES (?, ?, ?, ?, ?, ?)
        ''', (sno, title, file_name, json.dumps(quiz_json), question_count, difficulty))
        
        quiz_id = cursor.lastrowid
        conn.commit()
        logger.info(f"测验保存成功，ID: {quiz_id}，学号: {sno}")
        return quiz_id
    except Exception as e:
