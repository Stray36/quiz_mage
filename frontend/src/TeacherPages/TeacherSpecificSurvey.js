...
import { getQuizById, analyzeTeacherQuiz } from "../services/api";
...
export function TeacherSpecificSurveyPage() {
  const { quizId } = useParams();
  // 使用useHistory钩子获取路由历史对象
  ...
  async function onComplete(survey) {
    ...
    while (retryCount < MAX_RETRIES && !success) {
      try {
        ...
        // 发送答案到后端进行分析
        const result = await analyzeTeacherQuiz(survey.data, quizId);
        ...
        setTimeout(() => {
          ...
          // 跳转到特定分析页面
          history.push(`/analytics/${result.analysis_id}?tno=${tno}`);
        }, 3000); // 3秒后跳转
